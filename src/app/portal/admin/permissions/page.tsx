...
const ROLE_LABELS: Record<string, string> = { admin: '管理員', user: '使用者', dealer: '經銷商', viewer: '檢視者' }
const ROLE_COLORS: Record<string, string> = {
  admin: 'bg-red-500/20 text-red-400',
  user: 'bg-blue-500/20 text-blue-400',
  dealer: 'bg-amber-500/20 text-amber-400',
...
export default function PortalPermissionsPage() {
  ...
  const add = async () => {
    ...
    setSaving(true)
    setMsg('')
    const r = await fetch('/api/portal/permissions', {
      method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Portal 權限管理</h1>
        <p className="text-sm text-[var(--foreground-muted)]">管理 Portal 系統的使用者存取權限</p>
      </div>
