...
export async function GET() {
  ...
  const { data, error } = await supabase.from('hub_permissions').select('*').order('created_at', { ascending: false })
...
export async function POST(request: NextRequest) {
  ...
  const supabase = getSupabaseServerClient()
  const { data, error } = await supabase.from('hub_permissions').upsert({ user_email, role, updated_at: new Date().toISOString() }, { onConflict: 'user_email' }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
...
export async function DELETE(request: NextRequest) {
  ...
  const supabase = getSupabaseServerClient()
  const { error } = await supabase.from('hub_permissions').delete().eq('user_email', user_email)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
