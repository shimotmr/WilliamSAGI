...
import { buildIntelTaskPayload } from '@/lib/tag-intel-actions'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const mode = body?.mode === 'recommendation' ? 'recommendation' : 'hit'
    const payload = buildIntelTaskPayload({
      mode,
      label: body?.label,
      title: body?.title,
      url: body?.url,
      ...
      query: body?.query,
      reason: body?.reason,
      trackingGroup: body?.trackingGroup,
      linkedDomain: body?.linkedDomain,
    })
    ...
    try {
      await supabase.from('task_events').insert({
        task_id: taskId,
        event_type: 'tag_intel_task_created',
        content: {
          source: 'tag-intel',
          mode,
          label: body?.label || body?.title || null,
          url: body?.url || null,
          trackingGroup: body?.trackingGroup || null,
          linkedDomain: body?.linkedDomain || null,
        },
      })
    } catch {
      // non-blocking
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : 'Tag Intel 建卡失敗' },
      { status: 500 }
    )
  }
}
