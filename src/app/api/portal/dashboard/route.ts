...
export async function GET() {
  ...
  const [
    ...
    // 各階段案件數 + 金額
    getSupabase().from('cases').select('stage,amount,expected').neq('stage','取消'),
    // 本月目標 + 業績
    getSupabase().from('targets').select('target_amount').eq('year', y).eq('month', m),
    // 最近更新案件
    getSupabase().from('cases').select('id,end_customer,dealer,stage,amount,ship_date,rep').order('updated_at',{ascending:false}).limit(5),
    // 本月報價
    getSupabase().from('quotations').select('status,total_amount').gte('quote_date', monthStart),
  ])
