...
export default function HubLayout({ children }: { children: React.ReactNode }) {
  return (
    <ThemeProvider defaultContext="hub" defaultMode="dark">
      <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--background)', color: 'var(--foreground)', fontFamily: 'Inter, system-ui, sans-serif' }}>
        {/* Sidebar */}
          ...
          <nav style={{ flex: 1, padding: '1rem 0.75rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
            {navLinks.map(link => (
              <a key={link.href} href={link.href} style={{
                ...
                color: 'rgba(237,237,239,0.7)', textDecoration: 'none',
                fontSize: '0.8125rem', fontWeight: 500,
                transition: 'all 150ms ease',
              }}
              onMouseEnter={e => {
                (e.currentTarget as HTMLElement).style.background = 'rgba(94,106,210,0.12)';
                (e.currentTarget as HTMLElement).style.color = '#EDEDEF';
              }}
              onMouseLeave={e => {
                (e.currentTarget as HTMLElement).style.background = 'transparent';
                (e.currentTarget as HTMLElement).style.color = 'rgba(237,237,239,0.7)';
              }}>
