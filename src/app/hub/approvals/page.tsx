              ...
              <div key={item.id} className="bg-white rounded-xl shadow p-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-2 shrink-0">
                    <a href={`/api/hub/approve?id=${item.id}&action=approve`}
                      className="bg-green-600 text-white text-xs px-3 py-1.5 rounded-lg">批准</a>
                    <a href={`/api/hub/approve?id=${item.id}&action=reject`}
                      className="bg-red-500 text-white text-xs px-3 py-1.5 rounded-lg">退回</a>
                  </div>
                </div>
              </div>
            ))}
