...
const roleStyle: Record<CodexSessionMessage['role'], string> = {
  user: 'border-cyan-400/25 bg-cyan-400/10 text-cyan-50',
  assistant: 'border-emerald-400/20 bg-emerald-400/10 text-emerald-50',
  system: 'border-white/10 bg-white/5 text-zinc-200',
  tool: 'border-amber-400/20 bg-amber-400/10 text-amber-50',
  event: 'border-fuchsia-400/20 bg-fuchsia-400/10 text-fuchsia-50',
}

const roleLabel: Record<CodexSessionMessage['role'], string> = {
  user: 'USER',
  assistant: 'ASSISTANT',
  system: 'SYSTEM',
  tool: 'TOOL',
...
function extractAssistantTextFromTurn(params?: Record<string, unknown>): string {
  ...
  return items
    .map((item) => extractAgentMessageText(item).trim())
    .filter(Boolean)
    .join('\n')
}
...
export default function CodexPage() {
  ...
  async function loadDetail(id: string, options?: { clearLive?: boolean }) {
    ...
    try {
      const response = await fetch(`/api/codex/sessions/${encodeURIComponent(id)}`, {
        cache: 'no-store',
      })
      const payload = await response.json()
  ...
  function replaceRemoteThreads(threads: unknown) {
    ...
    const nextThreads = threads.reduce<Record<string, RemoteThread>>((accumulator, item) => {
      ...
      accumulator[id] = {
        ...
        updatedAt:
          typeof record.updatedAt === 'number' || typeof record.updatedAt === 'string'
            ? record.updatedAt
            : null,
      }
      return accumulator
    ...
    setLiveMessages((current) => {
      ...
      if (index >= 0) {
        nextMessages[index] = {
          ...nextMessages[index],
          id: assistantId,
          text: `${nextMessages[index].text}${chunk}`,
          streaming: true,
        }
        return nextMessages
      }
  ...
  function handleSocketMessage(event: MessageEvent<string>) {
    ...
    try {
      payload = JSON.parse(event.data) as RpcResponse
    } catch {
      return
    }
    ...
    if (payload.method === 'thread/started') {
      mergeRemoteThread(payload.params?.thread)
      return
    }

    if (payload.method === 'thread/status/changed') {
      const threadId =
        typeof payload.params?.threadId === 'string' ? payload.params.threadId : null
      const statusType =
        payload.params?.status && typeof payload.params.status === 'object'
          ? ((payload.params.status as { type?: string }).type ?? 'unknown')
          : 'unknown'
    ...
    if (payload.method === 'turn/started') {
      const threadId =
        typeof payload.params?.threadId === 'string' ? payload.params.threadId : null
      const turnId =
        payload.params?.turn && typeof payload.params.turn === 'object'
          ? ((payload.params.turn as { id?: string }).id ?? null)
          : null
    ...
    if (
      payload.method.includes('agentMessage') &&
      payload.method.includes('delta') &&
      typeof payload.params?.threadId === 'string' &&
      payload.params.threadId === attachedThreadIdRef.current
    ) {
      const turnId =
    ...
    if (payload.method === 'item/completed') {
      ...
      if (completedText) {
        finalizeLiveAssistant(completedText)
      }
      return
    }
    ...
    if (payload.method === 'turn/completed') {
      ...
      if (finalText) {
        finalizeLiveAssistant(finalText, activeTurnIdRef.current)
      }
      ...
      if (currentSelectedId) {
        void loadDetail(currentSelectedId, { clearLive: true })
      }
    }
  }
  ...
  }, [selectedId, attachedThreadId])

  useEffect(() => {
    attachedThreadIdRef.current = attachedThreadId
  }, [attachedThreadId])
  ...
  async function loadRuntimeInfo() {
    try {
      ...
      setRuntimeInfo({
        ...
        appServerListenUrl: APP_SERVER_URL,
        diagnostics: ['runtime probe 失敗，預設走 SSE fallback。'],
      })
    }
  }
  ...
  function sendRpc(
    method: string,
    params: Record<string, unknown>,
    socketOverride?: WebSocket,
  ) {
    const socket = socketOverride || socketRef.current
    if (!socket || socket.readyState !== WebSocket.OPEN) {
      return Promise.reject(new Error('WebSocket 尚未連線'))
    }
  ...
  function subscribeToSessionStream(sessionId: string) {
    ...
    source.addEventListener('delta', (event) => {
      ...
      const currentSelectedId = selectedIdRef.current
      if (currentSelectedId) {
        void loadDetail(currentSelectedId)
      }
    })

    source.addEventListener('error', () => {
      setConnectionError('SSE 連線中斷，請重新 Resume。')
  ...
  async function connectToAppServer() {
    ...
    connectPromiseRef.current = new Promise<void>((resolve, reject) => {
      ...
      socket.onerror = () => {
        setConnectionState('error')
        setConnectionError(`無法連到 ${APP_SERVER_URL}`)
        connectPromiseRef.current = null
        reject(new Error(`無法連到 ${APP_SERVER_URL}`))
      }
      socket.onclose = () => {
        socketRef.current = null
        setConnectionState('disconnected')
        pendingRequestsRef.current.forEach((pending) => pending.reject(new Error('WebSocket 已關閉')))
      ...
      socket.onopen = async () => {
        ...
        try {
          await sendRpc(
            ...
            {
              protocolVersion: 2,
              clientInfo: { name: 'codex-web', version: 'phase-2' },
          ...
          setConnectionState('error')
          setConnectionError(
            socketError instanceof Error ? socketError.message : '初始化 Codex app-server 失敗',
          )
          connectPromiseRef.current = null
          reject(
            socketError instanceof Error
              ? socketError
              : new Error('初始化 Codex app-server 失敗'),
          )
        }
      }
    })
  ...
  async function resumeSelectedSession() {
    ...
    try {
      ...
      setConnectionError(
        resumeError instanceof Error ? resumeError.message : '接手 session 失敗',
      )
    } finally {
      setResuming(false)
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!selectedId) {
      setConnectionError('目前沒有可接手的 session')
      return
    }

    if (attachedThreadId !== selectedId) {
    ...
    try {
      if (runtimeInfo?.mode === 'sse') {
        const response = await fetch(`/api/codex/sessions/${encodeURIComponent(selectedId)}/send`, {
          method: 'POST',
      ...
      const result = await sendRpc('turn/start', {
        threadId: selectedId,
      ...
      setLiveMessages((current) =>
        current.map((message) =>
          message.id.startsWith('assistant:')
            ? {
                ...message,
                text: turnError instanceof Error ? turnError.message : '送出失敗',
                streaming: false,
              }
            : message,
        ),
  ...
  const filteredSessions = useMemo(() => {
    ...
    return sessions.filter((session) =>
      [session.title, session.cwd, session.preview, session.model, session.provider]
        .filter(Boolean)
    ...
    <div className="min-h-screen bg-[#05070b] text-zinc-100 [font-family:var(--font-mono)]">
      <div className="mx-auto flex min-h-screen max-w-[1600px] flex-col lg:flex-row">
        ...
        <aside
          ...
          } w-full border-b border-white/10 bg-[#080b10] lg:block lg:h-screen lg:w-[360px] lg:border-b-0 lg:border-r`}
        >
          <div className="sticky top-0 border-b border-white/10 bg-[#080b10]/95 px-4 py-4 backdrop-blur">
            ...
            <label className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/30 px-3 py-2 text-sm text-zinc-400 focus-within:border-cyan-300/40">
              ...
              <input
                ...
                placeholder="搜尋 session / cwd / model"
                  ...
                  return (
                    <button
                      ...
                      className={`w-full rounded-2xl border p-3 text-left transition ${
                        active
                          ? 'border-cyan-300/40 bg-cyan-400/10 shadow-[0_0_0_1px_rgba(34,211,238,0.15)]'
                    ...
                    >
                      ...
                      {session.preview ? (
                        <p className="mt-3 line-clamp-2 text-xs leading-5 text-zinc-400">{session.preview}</p>
                      ) : null}
                    </button>
                  )
                })}
        ...
        <main className="flex min-h-[60vh] flex-1 flex-col relative pb-[200px] lg:pb-0">
          <div className="border-b border-white/10 bg-[#0a0e14] px-4 py-4 sm:px-6">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              ...
              <div className="flex flex-col gap-3 xl:items-end">
                <div className="flex flex-wrap gap-2 text-[11px] text-zinc-400">
                  {detail?.cwd ? (
                    <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
                      <FolderOpen className="h-3.5 w-3.5" /> {detail.cwd}
                    </span>
                  ) : null}
                  {detail?.source ? (
                    <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
                      <Laptop2 className="h-3.5 w-3.5" /> {detail.source}
                    </span>
                  ) : null}
                  {detail?.originator ? (
                    <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
                      <Server className="h-3.5 w-3.5" /> {detail.originator}
                    </span>
                  ) : null}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    onClick={() =>
                      connectionState === 'ready' ? refreshRemoteThreads() : connectToAppServer()
                    }
                    className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-zinc-200 transition hover:border-cyan-300/30 hover:text-cyan-200"
                  >
                  ...
                  <button
                    ...
                    disabled={!selectedId || connectionState === 'connecting' || resuming}
                    className="inline-flex items-center gap-2 rounded-2xl border border-cyan-400/20 bg-cyan-400/10 px-3 py-2 text-sm text-cyan-100 transition hover:border-cyan-300/40 hover:text-cyan-50 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {resuming ? <Loader2 className="h-4 w-4 animate-spin" /> : <Server className="h-4 w-4" />}
                    {attachedToSelected ? 'Attached' : 'Resume'}
                  </button>
                  <button
                    onClick={disconnectFromAppServer}
                    disabled={connectionState === 'disconnected'}
                    className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-zinc-300 transition hover:border-white/20 disabled:cursor-not-allowed disabled:opacity-50"
                  >
            ...
            <div className="mt-4 grid gap-2 text-xs text-zinc-400 sm:grid-cols-3">
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2">
                連線狀態：<span className="text-white">{connectionState}</span>
                <span className="ml-2 text-zinc-500">({runtimeInfo?.mode || 'probing'})</span>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2">
                Session 狀態：
                <span className="text-white"> {selectedRemoteThread?.statusType || 'not-connected'}</span>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2">
                Remote 更新：
                <span className="text-white"> {formatRuntimeTime(selectedRemoteThread?.updatedAt)}</span>
              </div>
            </div>
          </div>
          ...
          >
            {error ? (
              <div className="rounded-3xl border border-red-400/20 bg-red-500/10 px-4 py-3 text-sm text-red-200">{error}</div>
            ) : null}

            {connectionError ? (
              <div className="mb-4 rounded-3xl border border-amber-400/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
                {connectionError}
              </div>
            ) : null}
            ...
            {runtimeInfo?.diagnostics?.length ? (
              <div className="mb-4 rounded-3xl border border-cyan-400/20 bg-cyan-500/10 px-4 py-3 text-sm text-cyan-50">
                ...
                <ul className="space-y-1 text-sm text-cyan-100/90">
                  {runtimeInfo.diagnostics.map((item) => (
                    <li key={item}>• {item}</li>
                  ))}
                </ul>
              </div>
            ...
            {loadingDetail ? (
              ...
              <div className="space-y-3 pb-4">
                ...
                {displayedMessages.map((message) => (
                  <article
                    key={message.id}
                    className={`rounded-3xl border p-4 ${roleStyle[message.role]}`}
                  >
                    <div className="mb-3 flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.2em] text-white/70">
                      <span>{roleLabel[message.role]}</span>
                      <span className="text-white/35">{message.kind}</span>
                      <span className="text-white/35">{formatTime(message.timestamp)}</span>
                      {message.streaming ? <span className="text-cyan-200">streaming</span> : null}
                    </div>
                    <pre className="overflow-x-auto whitespace-pre-wrap break-words text-sm leading-6 text-inherit">
          ...
          <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-white/10 bg-[#080b10] px-4 py-4 sm:px-6 lg:static lg:z-auto">
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-400">
                <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5">
                  {attachedToSelected ? `Attached to ${selectedId}` : '尚未接手 session'}
                </span>
                <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5">
                  {runtimeInfo?.mode === 'sse'
                    ? 'Input: POST /send -> codex resume + tail JSONL'
                    : 'Input: turn/start -> [{ type: "text", text: "..." }]'}
                </span>
              </div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row">
                <textarea
                  ...
                  placeholder={
                    runtimeInfo?.mode === 'sse'
                      ? 'Resume 後會開始 tail session 檔案；送出訊息會背景呼叫 codex resume。'
                      : '先 Connect，再 Resume 選定 session，然後把接手訊息送進 Codex app-server。'
                  }
                  className="min-h-[112px] flex-1 rounded-[28px] border border-white/10 bg-black/30 px-4 py-3 text-sm leading-6 text-zinc-100 outline-none transition placeholder:text-zinc-600 focus:border-cyan-300/40"
                />
                <div className="flex w-full flex-col gap-3 lg:w-[220px]">
                  <button
                    ...
                    onClick={resumeSelectedSession}
                    disabled={!selectedId || connectionState === 'connecting' || resuming}
                    className="inline-flex items-center justify-center gap-2 rounded-[24px] border border-cyan-400/20 bg-cyan-400/10 px-4 py-3 text-sm font-medium text-cyan-100 transition hover:border-cyan-300/40 hover:text-cyan-50 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {resuming ? <Loader2 className="h-4 w-4 animate-spin" /> : <Server className="h-4 w-4" />}
                    {attachedToSelected ? 'Resume 完成' : 'Resume Session'}
                  </button>
                  <button
                    type="submit"
                    disabled={
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-[24px] border border-emerald-400/20 bg-emerald-400/10 px-4 py-3 text-sm font-medium text-emerald-100 transition hover:border-emerald-300/40 hover:text-emerald-50 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                    {sending ? 'Streaming...' : runtimeInfo?.mode === 'sse' ? 'Send via SSE' : 'Send via WebSocket'}
                  </button>
                </div>
              </div>
            </form>
      ...
      {showScrollBtn && (
        <button
          ...
          className="fixed bottom-[220px] right-4 z-40 flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-[#0c1018]/90 text-zinc-300 shadow-lg backdrop-blur transition hover:border-cyan-300/40 hover:text-cyan-200 lg:bottom-6"
          aria-label="捲動到底部"
        >
